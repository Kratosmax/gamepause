...
Game Pause 是一个面向 Windows 10/11 的游戏进程暂停工具，当前版本为 **1.2.3**。它可以批量暂停和恢复进程树，并提供游戏档案、自动规则、全局快捷键、托盘控制、异常恢复记录和独立守护进程。

本项目只面向单机游戏或明确可暂停的本地进程，不注入游戏、不修改游戏内存数据，也不尝试绕过反作弊。

> **重要升级提示：** 0.9.1 至 1.2.0 均无法依靠自身自动更新安装 1.2.1。0.9.1 至 1.1.3 会在旧包预检阶段失败，1.1.4 至 1.2.0 的旧更新器会锁住自身依赖。请从 [GitHub Releases](https://github.com/Kratosmax/gamepause/releases) 手动安装一次 1.2.1；用户档案和设置不会因此删除，升级后后续自动更新恢复正常。1.2.0 已撤回为预发布版本。
...
```text
temp\release\GamePause-1.2.3-Full-Setup.exe
temp\release\GamePause-1.2.3-Lite-Setup.exe
temp\release\GamePause-1.2.3-Full.zip
temp\release\GamePause-1.2.3-Lite.zip
```

对应的展开目录为 `GamePause-1.2.3-Full\` 和 `GamePause-1.2.3-Lite\`。Full 使用 `win-x64` 自包含发布并保留全部框架语言资源；Lite 使用框架依赖发布，不随包附带额外框架语言目录，程序现有中文界面和框架默认英文仍可用。发布脚本从 Lite 目录运行打包后更新器自测，后续本地包验证也应优先使用 Lite。发布目录中的三个可执行程序和 `distribution-channel.txt` 必须一起分发。

### 发布与自动更新

推送与项目版本一致的标签（例如 `v1.2.3`）会触发 `.github/workflows/release.yml`。工作流会运行完整测试、生成四种 Windows x64 发行包、编译安装包、签名双通道更新清单，并创建 GitHub Release。
...
新会话或新设备应先完整读取仓库根目录的 [`CODEX_PROGRESS.md`](CODEX_PROGRESS.md)。它记录当前版本、工作状态、关键入口、安全边界、验证矩阵和下一步；读取后必须用 Git、版本源和相关源码校准，不能把历史快照当成实时事实。每次开发结束前同步更新该文件，才能让下一次接手无需重复询问项目背景。

### 接手前必须读取

让 AI 先读取以下内容，再提出或实施改动：

1. `CODEX_PROGRESS.md`，执行接手校准并确认当前工作、阻塞和最近验证状态。
2. 本 README，了解产品边界、构建方式和用户操作。
3. 与需求直接相关的 `src/` 代码及调用方，不根据类名猜测行为。
4. `tests/GamePause.CoreTests/Program.cs`，了解已有安全和故障测试。
5. `scripts/publish.ps1`、`scripts/New-UpdateManifest.ps1`、发布工作流与三个项目文件，确认版本号、四种产物和 Full/Lite 更新通道。
6. 仓库内的 `AGENTS.md`（如果后续添加），遵守项目级协作规则。
...
开始前先完整阅读 CODEX_PROGRESS.md、README.md、与需求有关的源码及现有测试，先用 Git 和版本源校准进度文件，再用代码证据说明当前行为和拟修改位置；查不到就明确说不知道，不要猜调用链。
...
9. 不要自行提交或推送，除非我明确授权。
10. 完成前更新 CODEX_PROGRESS.md，只保留当前有效状态、实际测试结果、剩余阻塞和下一步，不记录密钥、用户数据或聊天流水账。
```
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\publish.ps1
dotnet .\temp\release\GamePause-1.2.3-Lite\GamePause.Updater.dll --self-test
```
