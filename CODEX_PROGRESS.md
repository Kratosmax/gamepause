...
| 默认分支 | `main` |
| 当前版本 | `1.2.2`（发布候选，尚未发布） |
| 本地最新标签 | `v1.2.1` |
| 产品代码基线 | `v1.2.1` 后的 `1.2.2` 更新公告与下载进度发布候选；提交哈希以实时 Git 为准 |
| Git 身份 | 仓库级 `小火车 <kratosthemax@gmail.com>` |
| 技术栈 | .NET 8、WPF、Windows 10/11 x64 |
| 线上状态 | Actions `32400575499` 成功；`v1.2.1` 为 latest 正式版且 8 个资产、签名、哈希和公开路由已核验；`v1.2.0` 保留为预发布审计记录 |
...
用户已确认发布 `1.2.2`。最终 Full Setup、Lite Setup、Full ZIP、Lite ZIP 已生成，包内版本、通道、运行时关系和哈希已核验；线上原始 `1.2.1` Full/Lite 客户端均接受对应候选 ZIP。下一步推送 `main`，创建并推送 `v1.2.2` 标签，等待 Actions 成功后核验 8 个线上资产、签名、哈希、latest 路由和完整 `1.2.1 -> 1.2.2` 更新链。
